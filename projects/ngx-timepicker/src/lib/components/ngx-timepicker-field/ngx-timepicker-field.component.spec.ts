...
describe('NgxTimepickerFieldComponent', () => {
    ...
    describe('ngOnInit', () => {
        ...
        it('should call NgxTimepickerUtils.disableMinutes when hour changes and min/max are set', fakeAsync(() => {
            ...
            const hour = 2;
            const period = NgxTimepickerPeriods.AM;
        ...
        it('should not call NgxTimepickerUtils.disableMinutes when selectedHour is undefined', fakeAsync(() => {
            const spy = spyOn(NgxTimepickerUtils, 'disableMinutes');
            const minutes = [{time: 1, angle: 0}];
            const format = 12;
            const min = DateTime.fromObject({hour: 11, minute: 12});
    ...
    describe('Format', () => {
        ...
        it('should update defaultTime when change format dynamically', () => {
            const spy = spyOn(NgxTimepickerAdapter, 'formatTime');
            ...
            component.format = 12;
            expect(spy).toHaveBeenCalledWith('23:00', {locale: NgxTimepickerAdapter.DEFAULT_LOCALE, format: 12});
        });
    });
    ...
    describe('writeValue', () => {
        ...
        it('should update time when writeValue called ', fakeAsync(() => {
            ...
            expect(component.period).toBe(NgxTimepickerPeriods.PM);
        }));
    ...
    it('should change hour and emit timeChanged event', fakeAsync(() => {
        const hour: NgxTimepickerClockFace = {
            time: 1,
            angle: 30
        };
    ...
    it('should change minute and emit timeChanged event', fakeAsync(() => {
        const minute: NgxTimepickerClockFace = {
            time: 15,
            angle: 90
        };
    ...
    it('should change period end emit timeChanged event', fakeAsync(() => {
        ...
        tick();
        expect(component.period).toEqual(NgxTimepickerPeriods.PM);
        expect(timer).toBe(expected);
    }));
