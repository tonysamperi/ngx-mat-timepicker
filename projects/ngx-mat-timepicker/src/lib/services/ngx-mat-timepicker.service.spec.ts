...
describe("NgxMatTimepickerService", () => {
    ...
    it("should set default hour on startup", () => {
        expect(selectedHour).toEqual(defaultHour);
    });

    it("should set default minute on startup", () => {
        expect(selectedMinute).toEqual(defaultMinute);
    });
    ...
    it("should return default full time as string (hh:mm a or HH:mm)", () => {
        expect(timepickerService.getFullTime(12)).toMatch(/^12:00\sAM$/);
    ...
    it("should change default time", () => {
        ...
        expect(selectedHour).toEqual({...defaultHour, time: 11});
        expect(selectedMinute).toEqual({...defaultMinute, time: 15});
        ...
        expect(selectedHour).toEqual({...defaultHour, time: 0});
        expect(selectedMinute).toEqual({...defaultMinute, time: 0});
        expect(selectedPeriod).toBe(NgxMatTimepickerPeriods.AM);

        time = "15:00";
        timepickerService.setDefaultTimeIfAvailable(time, null, null, 24);

        expect(selectedHour).toEqual({...defaultHour, time: 15});
        expect(selectedMinute).toEqual({...defaultMinute, time: 0});
        expect(selectedPeriod).toBe(NgxMatTimepickerPeriods.AM);
    });
    ...
    it("should reset time if default time is invalid", () => {
        ...
        expect(selectedHour).toEqual({...defaultHour, time: 10});
        expect(selectedMinute).toEqual({...defaultMinute, time: 10});
        expect(selectedPeriod).toBe(NgxMatTimepickerPeriods.AM);
